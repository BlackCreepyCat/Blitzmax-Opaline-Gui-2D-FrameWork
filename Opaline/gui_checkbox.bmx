...
Type TCheckBox Extends TWidget
    ...
    Method Draw(px:Int=0, py:Int=0)
        ...
        Local textX:Int = ax + (rect.h/2) -  (TWidget.GuiTextWidth("N",True) / 2)
        Local textY:Int = ay + (rect.h/2) -  (TWidget.GuiTextHeight("N",True) / 2) + 2

        ' Draw check mark if checked
        If checked
			TWidget.GuiDrawSymbol(textX ,textY , "N", TEXT_STYLE_SHADOW, COLOR_CHECKBOX_MARK_R, COLOR_CHECKBOX_MARK_G, COLOR_CHECKBOX_MARK_B)
        EndIf

        ' Draw label text with shadow
        textX = ax + rect.h + 8
        textY = ay + (rect.h -  TWidget.GuiTextHeight(caption)) / 2
